...
' Constants - zLib (v1.1.3)
Private Const Z_NULL = 0
Private Const Z_PARTIAL_FLUSH = 1 ' Obselete - use Z_SYNC_FLUSH inStead
Private Const Z_SYNC_FLUSH = 2
Private Const Z_FULL_FLUSH = 3
Private Const Z_FINISH = 4

' Constants - zLib (v1.1.3) Error Values
Private Const Z_OK = 0               ' No Error
Private Const Z_STREAM_END = 1       ' Data Stream reached the end of the Stream
Private Const Z_NEED_DICT = 2        ' A preset dictionary is needed
Private Const Z_ERRNO = (-1)         ' A file syStem error has occured
Private Const Z_STREAM_ERROR = (-2)  ' A function parameter is invalid *OR* The Stream State was inconsiStent
Private Const Z_DATA_ERROR = (-3)    ' Input data was corrupted
Private Const Z_MEM_ERROR = (-4)     ' Not enough memory
Private Const Z_BUF_ERROR = (-5)     ' Not enough room in the output buffer
Private Const Z_VERSION_ERROR = (-6) ' The zlib library version is incompatible with the version assumed by the caller

' Constants - zLib (v1.1.3) Compression Strategy
Private Const Z_FILTERED = 1
Private Const Z_HUFFMAN_ONLY = 2
Private Const Z_DEFAULT_STRATEGY = 0

' Constants - zLib (v1.1.3) Possible values of the data_Type field
Private Const Z_BINARY = 0
Private Const Z_ASCII = 1
Private Const Z_UNKNOWN = 2

' Constants - zLib (v1.1.3) The deflate compression method (the only one supported in this version)
