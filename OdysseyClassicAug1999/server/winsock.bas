...
Public Function SendData(ByVal S&, St As String) As Long
    Dim TheMsg() As Byte
    BytesSent = BytesSent + Len(St)
    If BytesSent > 2000000000 Then BytesSent = 2000000000
    TheMsg = ""
...
Function Receive(S As Long) As String
    ...
    If L > 0 Then
        ...
        BytesReceived = BytesReceived + Len(St)
        If BytesReceived > 2000000000 Then BytesReceived = 2000000000
        Receive = St
    End If
End Function
...
Function ConnectSock(ByVal host$, ByVal Port&, retIpPort$, ByVal HWndToMsg&, Msg As Long, ByVal Async%) As Long
    ...
    Else
        ...
        End If
    End If
    If setsockopt(S, SOL_SOCKET, SO_RCVBUF, 1024&, 4) = SOCKET_ERROR Then
        If S > 0 Then
            dummy = closesocket(S)
        End If
        ConnectSock = INVALID_SOCKET
        Exit Function
    End If
    If setsockopt(S, SOL_SOCKET, SO_SNDBUF, 1024&, 4) = SOCKET_ERROR Then
        If S > 0 Then
            dummy = closesocket(S)
        End If
        ConnectSock = INVALID_SOCKET
